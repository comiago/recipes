<link rel="stylesheet" href="http://use.fontawesome.com/releases/v6.0.0/css/all.css" integrity="sha384-3B6NwesSXE7YJlcLI9RpRqGf2p/EgVH8BgoKTaUrmKNDkHPStTQ3EyoYjCGXaOTS" crossorigin="anonymous">
<header class="header">
  <a href="../../public/index.php"><h1 class="title">Ricette</h1></a>
  <div class="menu">
    <a href="../../public/index.php"><i class="fas fa-home"></i></a>
    <a href=""><i class="fas fa-search"></i></a>
    <a href="../../public/dashboard.php" class="access"><i class="fas fa-user-alt"></i></a>
  </div>
</header>
